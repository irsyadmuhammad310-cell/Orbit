...
// V1.5.2: Goal-Habit linking, Goal-Project linking, auto-progress from habits
...
var Goals = {
  render: function() {
    ...
    document.getElementById('pg-goals').innerHTML = '<div class="pg-h"><div class="back" onclick="App.go(\'more\')">←</div><div class="pg-t" style="flex:1">Goals</div><div class="pg-btn" onclick="Goals.openForm()">+</div></div>' +
      (active.length ? '<div class="sec">Active (' + active.length + ')</div>' + active.map(function(g) {
        var pct = Goals.calcProgress(g);
        var color = pct >= 75 ? 'var(--green)' : pct >= 40 ? 'var(--orange)' : 'var(--accent)';
        var linkedH = habits.filter(function(h) { return h.linkedGoalId === g.id; });
        var linkedP = g.linkedProjectId ? DB.get('projects', g.linkedProjectId) : null;
        return '<div class="goal" onclick="Goals.view(\'' + g.id + '\')"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><span style="font-size:22px">' + (g.emoji||'🎯') + '</span><div style="flex:1"><div style="font-size:14px;font-weight:700">' + DB.esc(g.name) + '</div><div style="font-size:11px;color:var(--text3)">' + (g.lifeArea||'') + (g.deadline ? ' · Due ' + g.deadline : '') + '</div></div><div style="font-size:18px;font-weight:800;color:' + color + '">' + pct + '%</div></div><div class="bar"><div class="fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
          (linkedH.length ? '<div style="margin-top:6px;display:flex;gap:4px;flex-wrap:wrap">' + linkedH.map(function(h) { var s = Habits.getStreak(h.id); return '<span style="font-size:10px;padding:2px 6px;border-radius:4px;background:var(--surface3)">' + h.icon + ' ' + (s>0?'🔥'+s:'') + '</span>'; }).join('') + '</div>' : '') +
          (linkedP ? '<div style="margin-top:4px;font-size:10px;color:var(--text3)">🚀 ' + DB.esc(linkedP.name) + '</div>' : '') +
          (g.milestones && g.milestones.length ? '<div style="margin-top:4px;font-size:10px;color:var(--text3)">' + g.milestones.filter(function(m){return m.done;}).length + '/' + g.milestones.length + ' milestones</div>' : '') +
        '</div>';
  ...
  // Calculate progress: milestones > linked habits avg > manual slider
  calcProgress: function(g) {
    // Priority 1: Milestones
    if (g.milestones && g.milestones.length) {
      var doneMs = g.milestones.filter(function(m) { return m.done; }).length;
      return Math.round(doneMs / g.milestones.length * 100);
    }
    // Priority 2: Linked habits average completion (30-day)
    var linkedH = DB.getAll('habits').filter(function(h) { return h.linkedGoalId === g.id; });
    if (linkedH.length) {
      var totalRate = linkedH.reduce(function(sum, h) { return sum + Habits.getCompletionRate(h.id, 30); }, 0);
      return Math.round(totalRate / linkedH.length);
    }
    // Priority 3: Manual
    return g.progress || 0;
  },
  ...
  saveNew: function() {
    ...
    DB.add('goals', { name: n, emoji: document.getElementById('ge').value || '🎯', progress: 0, deadline: document.getElementById('gd').value, lifeArea: document.getElementById('gla').value, linkedProjectId: document.getElementById('glp').value || null, milestones: milestones, completed: false, notes: '' });
  ...
  view: function(id) {
    ...
    var pct = Goals.calcProgress(g);
    var linkedHabits = DB.getAll('habits').filter(function(h) { return h.linkedGoalId === g.id; });
    var projOpts = DB.getAll('projects').map(function(p) { return '<option value="' + p.id + '"' + (g.linkedProjectId===p.id?' selected':'') + '>' + DB.esc(p.name) + '</option>'; }).join('');

    // Linked habits section
    var habitsHtml = '';
    if (linkedHabits.length) {
      habitsHtml = '<div class="sec">Supporting Habits</div>' + linkedHabits.map(function(h) {
        var streak = Habits.getStreak(h.id);
        var rate = Habits.getCompletionRate(h.id, 7);
        return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)"><span>' + h.icon + '</span><span style="flex:1;font-size:13px;font-weight:500">' + DB.esc(h.name) + '</span><span style="font-size:11px;color:var(--text3)">' + (streak>0?'🔥'+streak:'') + ' · ' + rate + '%/7d</span></div>';
      }).join('');
    }
    ...
    openSheet('<div class="sheet-title">' + (g.emoji||'🎯') + ' ' + DB.esc(g.name) + '</div>' +
      '<div style="text-align:center;margin-bottom:12px"><div style="font-size:32px;font-weight:800;color:var(--accent)">' + pct + '%</div><div style="font-size:10px;color:var(--text3)">' + (linkedHabits.length ? 'Based on ' + linkedHabits.length + ' habit(s)' : g.milestones.length ? 'Based on milestones' : 'Manual progress') + '</div></div>' +
      '<div class="fg"><label class="fl">Progress (manual override)</label><input type="range" id="ggp" min="0" max="100" value="' + (g.progress||0) + '" style="width:100%"></div>' +
      ...
      '<div class="fg"><label class="fl">Link to Project</label><select class="fs" id="gglp"><option value="">None</option>' + projOpts + '</select></div>' +
  ...
  saveEdit: function(id) {
    ...
    g.linkedProjectId = document.getElementById('gglp').value || null;
  ...
  del: function(id) {
    ...
    // Unlink habits
    DB.getAll('habits').forEach(function(h) { if (h.linkedGoalId === id) h.linkedGoalId = null; });
...
var Habits = {
  render: function() {
    ...
    document.getElementById('pg-habits').innerHTML = '<div class="pg-h"><div class="back" onclick="App.go(\'more\')">←</div><div class="pg-t" style="flex:1">Habits</div><div style="display:flex;gap:4px"><button class="pg-btn" style="font-size:14px" onclick="Habits.showAnalytics()">📊</button><button class="pg-btn" onclick="Habits.openForm()">+</button></div></div>' +
      ...
      (habits.length ? habits.map(function(h) {
        ...
        var linkedGoal = h.linkedGoalId ? DB.get('goals', h.linkedGoalId) : null;
        var cells = '';
        ...
        return '<div class="hab"><div class="hchk' + (done?' done':'') + '" onclick="Habits.toggle(\'' + h.id + '\')">✓</div><div class="hn">' + h.icon + ' ' + DB.esc(h.name) + (linkedGoal ? '<span style="font-size:9px;color:var(--text3);margin-left:4px">→ ' + (linkedGoal.emoji||'🎯') + '</span>' : '') + '</div><div class="hs">' + (str>0?'🔥'+str:'—') + '</div><div class="hgrid">' + cells + '</div><button style="border:none;background:none;color:var(--text3);font-size:14px;padding:4px;cursor:pointer" onclick="event.stopPropagation();Habits.del(\'' + h.id + '\')">✕</button></div>';
  ...
  showAnalytics: function() {
    ...
    habits.forEach(function(h) {
      ...
      var linkedGoal = h.linkedGoalId ? DB.get('goals', h.linkedGoalId) : null;
      html += '<div class="card" style="margin-bottom:10px"><div style="font-size:14px;font-weight:700;margin-bottom:8px">' + h.icon + ' ' + DB.esc(h.name) + (linkedGoal ? ' <span style="font-size:10px;color:var(--text3)">→ ' + DB.esc(linkedGoal.name) + '</span>' : '') + '</div>' +
  ...
  openForm: function() {
    var goalOpts = DB.getAll('goals').filter(function(g){return !g.completed;}).map(function(g) { return '<option value="' + g.id + '">' + (g.emoji||'🎯') + ' ' + DB.esc(g.name) + '</option>'; }).join('');
    openSheet('<div class="sheet-title">New Habit</div><div class="fg"><label class="fl">Habit name</label><input class="fi" id="hn" placeholder="Meditate 10 min"></div><div class="fr"><div class="fg"><label class="fl">Icon</label><input class="fi" id="hi" placeholder="⭐" style="text-align:center"></div><div class="fg"><label class="fl">Frequency</label><select class="fs" id="hf"><option value="daily">Daily</option><option value="weekdays">Weekdays</option><option value="3x_week">3x/week</option><option value="weekly">Weekly</option></select></div></div><div class="fg"><label class="fl">Link to Goal</label><select class="fs" id="hlg"><option value="">None</option>' + goalOpts + '</select></div><div class="fg"><label class="fl">Life Area</label><select class="fs" id="hla"><option value="">None</option><option>Health</option><option>Learning</option><option>Career</option><option>Personal</option></select></div><button class="btn" onclick="Habits.saveNew()">Add Habit</button>');
  },

  saveNew: function() {
    var n = document.getElementById('hn').value.trim();
    if (!vReq(n, 'Enter name')) return;
    DB.add('habits', { name: n, icon: document.getElementById('hi').value || '⭐', frequency: document.getElementById('hf').value, linkedGoalId: document.getElementById('hlg').value || null, lifeArea: document.getElementById('hla').value });
